...
public class MixinLootTable {
    ...
    private void processStacksDifferently(Args args, LootContext context, Consumer<ItemStack> lootConsumer) {
        ...
        for (var key : ctx.keySet()) {
            if (key == LootContextParameters.THIS_ENTITY) {
                if (ctx.get(key) instanceof CowEntity) {
                    args.set(0, processStacksCorrectly(lootConsumer));
                }
            }
        }
    }

    private static Consumer<ItemStack> processStacksCorrectly(Consumer<ItemStack> lootConsumer) {
        return (stack) -> {
